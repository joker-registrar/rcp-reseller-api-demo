...
class Domain
{
    ...
    function bulk_transfer_step3()
    {
        ...
        $cdata = array();
        $result = $this->tools->query_object("contact", $_SESSION["userdata"]["t_contact_billing"], true);
        if ($result) {
                ...
                foreach($result as $value)
                {
                    switch ($value["0"]) {
                        ...
                        case "contact.organization:":
                            $cdata["Organization"] = $value["1"];
                            break;
                        case "contact.address-1:":
                            $cdata["Address1"] = $value["1"];
                            break;
                        case "contact.address-2:":
                            $cdata["Address2"] = $value["1"];
                            break;
                        case "contact.city:":
                            $cdata["City"] = $value["1"];
                            break;
                        ...
                        case "contact.postal-code:":
                            $cdata["Postalcode"] = $value["1"];
                            break;
                        case "contact.country:":
                            $cdata["Country"] = $value["1"];
                            break;
                        case "contact.email:":
                            $cdata["Email"] = $value["1"];
                            break;
        ...
        if (count($cdata) == 0) {
            $this->tools->tpl->set_var("ERROR_MSG", $this->msg["_request_failed"]." ".$this->err_msg["_contact_hdl"]);
            $this->tools->tpl->parse("CONTENT", "general_error_box");
        }
        
        $billing_c =  "NAME: " .$cdata["Name"];
        $billing_c .= "\nNAME1: ".$cdata["Name1"];
        $billing_c .= "\nNAME2: ".$cdata["Name2"];
        $billing_c .= "\nORGANIZATION: ".$cdata["Organization"];
        $billing_c .= "\nADDRESS1: ".$cdata["Address1"];
        $billing_c .= "\nADDRESS2: ".$cdata["Address2"];
        $billing_c .= "\nPLZ: ".$cdata["Postalcode"];
        $billing_c .= "\nPHONE: ".$cdata["Phone"];
        $billing_c .= "\nFAX: ".$cdata["Fax"];
        $billing_c .= "\nCITY: ".$cdata["City"];
        $billing_c .= "\nSTATE: ".$cdata["State"];
        ...
        $subject = "Bulk transfer [".$_SESSION["uid"]."] - ".(count($_SESSION["userdata"]["domain_authid_pairs"]))." Domains";
        $email_body = "Domains, AUTHIDs:\n\n" . $domain_authid_list . "\n\nBilling-Contact:\n".$billing_c."\n\nNameserver:\n".$nserver."\n\nAutorenew: ".
                "no"."\n\nUser: ".$_SESSION["username"]."\nUser-Id:".$_SESSION["uid"].
                "\nRemote-IP:".$_SERVER["REMOTE_ADDR"]."\nDatum: ".gmdate("d\.m\.Y G:i:s", time())." generated by DMAPI";
        
        if ($this->tools->send_mail($this->config["transfer_email"], $this->config["dmapi_mp_email"], "", "", $subject, $email_body, "", "", "")) {
